 ...
 *
 * The dsh-* alternation mirrors the official 0.1.2 gate (packages/client/
 * tsdown.client.ts). `dsh-client-ui-conversation/client` is a deliberate fork
 * extension: this workspace replaces the official conversation plugin with
 * @ryanyujazz/dsh-client-ui-conversation, whose apply constructs the official
 * UiConversation service class — the only instantiation in the app, reached
 * by every other plugin through the cordis `uiConversation` service instead
 * of the class identity.
 */
export const INLINE_SAFE
  = /^(?:@deepseek-ai\/dsh-(?:file-reference|session|llm|tools|brand|deque|typert-protocol|util-crypto|util-values|util-workspace-path)(?:\/|$)|@deepseek-ai\/dsh-token-meter\/client$|@deepseek-ai\/dsh-agent-presets\/display$|@deepseek-ai\/dsh-client-ui-conversation\/client$)/
...
/**
 * Externals resolved from the loader module table. 0.1.2 rehomed the
 * snapshot-store engine into the published `@deepseek-ai/dsh-client-store`
 * platform row, retiring the former `dsh-client-runtime/client` exemption.
 */
  ...
  return {
    ...
    plugins: [{
      ...
      resolveId(source: string, importer?: string) {
        ...
        if (process.env.DSH_PURITY_TRACE) console.error(`[purity-trace] ${source} <- ${importer ?? '?'}`)
        throw new Error(
