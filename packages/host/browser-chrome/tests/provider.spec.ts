...
describe('System Chrome Provider', () => {
  ...
  it('ships a fixed-ID MV3 extension with explicit tab sharing and no remote-debugging port', async () => {
    ...
    expect(worker).toContain('stableLocators')
    expect(worker).toContain('navigationRevisions')
    expect(worker).toContain('popupPolicy')
    expect(worker).toContain('POPUP_BLOCKED')
    expect(worker).toContain("command.popupPolicy==='deny')popupUrl=element.href")
    expect(worker).toContain('AMBIGUOUS_LOCATOR')
    expect(worker).toContain('POSTCONDITION_TIMEOUT')
    expect(worker).toContain('command.steps')
    expect(worker).toContain('command.expectedUrl')
    expect(worker).not.toContain("tab.url?.includes(command.value||'')")
