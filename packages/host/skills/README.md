...
The eleven official directories plus DeepCreator's three browser-control Skills ship verbatim under `assets/skills/` (SKILL.md plus each skill's `agents/`, `references/`, and `scripts/` files). The provider parses each SKILL.md frontmatter with the official filesystem provider's semantics — `name`/`description` required, `disable-model-invocation` and `user-invocable` flags honored — serves the body with the packaged directory as the resource base, and registers with `BUNDLED_SKILL_RANK` so user-owned definitions override it through the ordinary registry precedence.

English catalog descriptions remain authoritative in the verbatim `SKILL.md` files. The provider adds matching Chinese descriptions as `metadata.localizedDescriptions`, exclusively for bilingual management UI; the metadata does not alter model-facing instructions. It also declares `metadata.developer` separately from the provider: the eleven upstream development Skills belong to `DeepSeek Harness`, while the three browser-control Skills belong to `DeepCreator`.
