...
1. Invoke `dsh-browser-control` and follow its presentation, interaction, and cleanup instructions. Prefer a visible IAB tab created through `open_in_deepcreator` when the recording should mirror what the user sees; use the managed headless Provider only for isolated background capture. Use the user's existing Chrome state only when explicitly requested or required, and state that exception in the provenance. If no managed Provider is available, report the limitation instead of installing another driver or launching a personal browser.
