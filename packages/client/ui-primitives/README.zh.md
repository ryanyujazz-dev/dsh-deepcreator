...
`DiffBlock` 通过共享的 `jsdiff` 行模型渲染上下文 hunk。同一路径的所有 hunk 合成为一张圆角文件卡片，只显示一次完整路径与汇总后的 `+A -R`；Review 可关闭该头部，因为外层文件行已经持有同一文件身份。单行号槽在删除行显示旧行号，在新增与上下文行显示新行号；独立符号列保持 `+`／`-` 对齐。内容使用 `pre-wrap` 和安全断词，窄面板软换行时不会重复 gutter。替换行通过 `diffWordsWithSpace()` 生成词级 mark，同时保留 Shiki token；超大替换块只跳过词级细化。绝对起始行可选，旧回放缺失时行号有意留空。新增、删除与替换行不会再被通用行数上限隐藏；只有未修改上下文可以折叠：长 context 保留前后各三行，并在原位置插入「⋯ 展开 N 行」FoldRow。Review 省略的文件头、hunk 间隔和文件尾初始只保留轻量 `{ source, start, count }` 范围，用户打开 FoldRow 后才生成对应的行、React 与语法模型。父组件可控制折叠键以跨视口卸载保存展开状态，未传入时仍保持原有非受控兼容。源码快照高亮可渐进执行和取消，缓存改为约 32 MiB 加权 LRU。命名代码主题同时提供语法前景和半透明的行级／词级 Diff 表面；Review 变体保持 hunk 基础背景透明，以透出 Workbench 自身的应用表面。强制颜色与高对比覆盖始终最后生效。
