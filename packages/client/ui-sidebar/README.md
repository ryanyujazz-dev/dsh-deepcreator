...
Sidebar shell plugin: the wordmark, the stage-mode seat, marginless primary-action list, collapse/reopen controls, scroll-aware region seat, and bottom-pinned footer seats. Directly under the brand row the shell declares the `sidebar.stage-mode` seat (single, root scope) for the 对话｜应用 stage-mode segmented control — [ui-layout](../ui-layout/README.md) contributes the control there because stage mode is layout state; this package owns only the position (outside the primary action list, shell chrome not a feature action) and passes the `wide` column fact. The primary list uses the same `SidebarRow` geometry as Workspace/project and Session titles: shell-owned New Session is followed by feature rows from the additive `sidebar.primary.action` Slot and the disabled Scheduled Tasks placeholder, with list-owned 2px row rhythm and no vertical outer margin. [ui-skills](../ui-skills/README.md) currently contributes Skills; [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`. This package derives neither feature's rows or view state. Closing removes the sidebar surface completely; this package contributes the sole reopen control into layout's `deepcreator.shell.sidebar-toggle` frame seat. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
New Session starts the runtime's page-local frontend Session Intent. The runtime targets the explicit Workspace used by a scoped action, otherwise the current Session's Workspace, otherwise the most recently active Workspace; when none exists it clears into the blank New Session page. Feature-owned primary actions receive only the `wide` presentation fact; Skills opens its settings section through the settings navigation service. Scheduled Tasks remains presentation-only and disabled. Workspace-specific controls and the shared picker belong to ui-workspace.

`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.stage-mode`, `sidebar.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
...
The foot stacks additive `sidebar.footer.action` contributions above the `sidebar.settings` seat and shares the column state (`wide`) with both. Footer contributors are expected to use the shared `SidebarRow` grid. The shell applies a narrow compatibility bridge to the retained official Cordis trigger's stable `data-cordis-badge` marker, normalizing its height, typography, spacing, radius, and glyph metric without matching generated classes or reaching its nested panel. Cordis still owns its copy, state, behavior, runtime, and panel; ui-settings registers the Settings trigger and panel in its dedicated seat.
