...
The shell renders the DeepCreator wordmark in its 48px brand row. Standard sidebar action glyphs use ui-primitives' shared 14px metric in both expanded and collapsed states; the 16px whale is an optical brand exception centered on the same axis, and button hit areas retain their feature-owned geometry.
