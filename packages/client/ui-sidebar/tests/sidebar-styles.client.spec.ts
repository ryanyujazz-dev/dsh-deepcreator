...
describe('SidebarRoot.module.css', () => {
  ...
  it('normalizes contributed footer triggers to the shared sidebar row grid', () => {
    const foot = declarations('.footArea')
    const actions = declarations('.footArea > .footerActions')
    const contribution = declarations('.footArea .footerActions > :has(button[data-cordis-badge])')
    const trigger = declarations('.footArea .footerActions button[data-cordis-badge]')
    const icon = declarations('.footArea .footerActions button[data-cordis-badge] > svg')
    const copy = declarations('.footArea .footerActions button[data-cordis-badge] > span')

    expect(foot?.get('gap')).toBe('2px')
    expect(actions?.get('flex-direction')).toBe('column')
    expect(actions?.get('gap')).toBe('2px')
    expect(contribution?.get('height')).toBe('var(--dsw-sidebar-row-height, 32px)')
    expect(contribution?.get('margin')).toBe('0')
    expect(trigger?.get('height')).toBe('var(--dsw-sidebar-row-height, 32px)')
    expect(trigger?.get('padding')).toBe('0 8px')
    expect(trigger?.get('border-radius')).toBe('8px')
    expect(trigger?.get('gap')).toBe('6px')
    expect(trigger?.get('font-size')).toBe('var(--dsw-font-sidebar-font-size, 12px)')
    expect(trigger?.get('line-height')).toBe('var(--dsw-font-sidebar-line-height, 18px)')
    expect(icon?.get('width')).toBe('14px')
    expect(icon?.get('height')).toBe('14px')
    expect(copy?.get('font-size')).toBe('inherit')
    expect(copy?.get('line-height')).toBe('inherit')
  })
})
