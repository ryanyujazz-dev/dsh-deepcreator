...
会话领域：骨架（标题栏／标签页／编辑器／空状态）、聊天视图（分组步骤摘要流、流式尾部隔离与轮次状态）、编辑器 dock（与输入区一同 sticky 的会话统计行）、输入区 dock（队列行加 todo 计划条），以及按 scope 寻址的 ConversationController。工具展示属于 [`ui-tool`](../ui-tool/README.md)。
...
Harness V3 的 `system/message` 表层事件会被 Chat 接管但刻意不显示，因此系统提示词内部信息不会落入“未知事件”渲染器。可审查的系统提示词与 `request/header` 由轨迹视图负责呈现，包括表层替换或压缩后的提示词变化。
...
经典和思考模式的执行槽会把工具生命周期结算视为一次消息流更新，即使带 key 的节点顺序没有变化也一样。`tool/result` 一旦把运行中节点变为已结算，聚合标题就会立即推进并播放正常的完成过渡，不再等待下一条工具草稿、后续调用或 Turn 尾部来使消息流失效。
...
**内嵌子会话。** `deepcreator.conversation.embed` 现在只是几何适配层：它把子会话 id 交给 Runtime 的显式非导航 `SessionProvider`，再由严格会话 surface 以 `{ transcriptOnly: true, surfaceId }` 调用主对话区同一个已授权 `conversation.session` outlet。两个区域共享同一个 Session、官方尾页／open promise、assembler、实时流、分页、主对话区的渲染模式偏好、节点／工具 renderer 与 Host 排版变量；Activity 不挂载 composer、不提供第二个模式切换入口，也不维护事件镜像。滚动位置按会话和 surface 分开保存，历史图片 URL 则使用引用计数租约，任一区域卸载都不会让另一区域资源失效。Activity 冷挂载直接物化主对话区从官方首次尾页渲染的同一组完整驻留 Turn，不再额外施加“只显示一轮”的展示预算。公共「加载更早」也直接委托 Runtime 的真实 `loadOlder`，因此两个位置的首次窗口与分页语义完全一致。非当前会话最后一个观察者退出时 Runtime 停止实时处理但保留有界的已组装尾页；重进仍会重取官方尾页，仅当事件序列未变化时复用展示快照。根区域只发布 renderer capability，从不复制 Session 数据；Activity 仅在标签激活、面板可见且页面可见时持有观察租约。
...
键盘消息提交会根据所寻址会话的运行状态和 steering 能力解析投递方式；它的通用设置行固定排在「语言」下方、「偏好」标题上方。空闲时，Enter 和 Cmd/Ctrl+Enter 都执行普通 Queue 发送。主会话或可继续的 subagent 运行期间，由 Host settings 支撑的 `ui-conversation.busyEnter` 偏好会把普通 Enter 分配为 `Queue`（默认值）或 `Steer`，Cmd/Ctrl+Enter 则执行另一种行为；本地 settings 提供方将其存入 `$DSH_HOME/settings.yaml`，因此该选择会跟随同一个用户 home 跨越 Web 端口。Shift+Enter 仍然换行。草稿为空时，Cmd/Ctrl+Enter 改为按 FIFO 顺序把仍在排队的消息全部插话进运行中的轮次；空草稿 + 普通 Enter 仍是无操作。这个整队列手势可用时，文本框 placeholder 会提示该手势；owner 提供的 placeholder 仍然优先。不可继续的 subagent 保留仅 Queue 的传输。主发送按钮会跟随同一份实时繁忙偏好，并明确显示「排队发送」或「插话发送」，使指针与键盘提交一致。Composer Steer 复用现有尽力而为的 `session.prompt(mode: 'steer')` 约定：如果当前 next-step 窗口在接纳前关闭，AgentLoop 会把消息接纳为下一条唤醒 Queue 轮次，不显示失败，也不会丢失草稿事务。该持久化边界由[Host settings 支撑的偏好决策](../../../.agents/notes/implemented/bug-fix/2026-08-06-host-backed-web-preferences.md)拥有。

普通提交会在乐观清空草稿与官方 Session 投影真正可见之间创建一条纯展示层的浏览器本地回显。它按已解析的投递方式进入对话尾部、QueueDock 或 Steer 区域，按 FIFO 与匹配的权威 Queue 或持久聊天内容配对，并仅在所属界面提交该后继节点后退场。它永不持久化、回放或提升为 Session 业务状态；提交被拒绝时会撤回回显，并保留既有的未改动草稿恢复行为。

逐会话 UI 状态中的选择与活跃视图位于已声明的聊天 store（`stores.ts` `createChatStore`）中；InputHub 拥有输入区状态机，并将草稿镜像到该 store 以便持久化。apply 将同一个 store handle 传给严格限定于会话的子树和聊天视图，因此每个会话内共享一个实例，框架拥有其生命周期。组件保持纯粹：框架标准工具包提供 `useSession`／`sessionId`、全局 `useSessions`／`useWorkspaces`，以及输入状态机的 `useInput`／`inputActions`；store 表层与 inject factory 提供其余状态和回调。

附件呈现继续归保留的官方 attachment 插件所有。ui-conversation 只声明稳定的 `conversation.input.attachments` 与 `conversation.message.images` seat，并经 owner props 提供 composer／消息状态和图片加载能力；它不再 import 附件 React 组件。InputBar 只拥有加号旁原生多文件选择器的触发按钮，并把选中的 `File` 交给公共 attachment owner。图片保留浏览器预览并在发送时编码；通用文件在选取后立即经官方 `fileUpload` 后台上传，把进度与重试状态交给附件栏，发送时只提交持久化 receipt。粘贴、整页拖放、预览、画廊和灯箱因而都保持官方行为。宿主侧拒绝按 `attachment-error` 原因映射为产品文案（`image-labels.ts` 的 `attachmentErrorText`）；用户无法解决的原因折叠为一条带原因码的发送失败文案，非附件错误码保留开发者可读的原文加错误码。

输入栏为 `'conversation.input.plan'`（位于本地 access 模式控件右侧）和 `'conversation.input.model'`（渲染在 pending 指示器与发送／停止控件之前）声明会话作用域的单实例 seat，并为 overlay、dock、left 和 right 输入扩展声明列表 slot。各功能包拥有相应控件及其状态；ui-conversation 提供放置位置、`locked` owner prop 和标准 slot share。前置加号按钮是 Command launcher：它要求当前会话的 `InputTriggerController` 基于 textarea 当前 selection，只打开 `/` trigger 的 `command` source；相邻回形针打开原生附件选择器，并把选择结果交给官方 attachment owner，粘贴和整页拖放仍由 attachment 插件提供。当 `plan` 投影的有效目标为 plan mode 时，InputBar 将文本框 placeholder 切换为 plan 任务措辞，经本包注册的 `conversation` locale 命名空间（`placeholder.plan` / `hint.plan` 键）本地化，并与已认领 `/plan` 命令的提示逐字共用同一份文案（经标准套件 `useProjection` 读取的 host 折叠值；owner 提供的 placeholder 优先）。另一个会话视图活跃时，待处理的 composer 接管仍保持挂载，使被阻塞的 agent（智能体）仍能收到回答；没有待处理交互时，活跃会话的 composer 归 Chat 所有。composer bar slot 本身为 `session-maybe`：没有当前会话时，同一个 bar 会让消息操作保持不可交互（machine face 均缺席、`disabled` owner prop），整张虚线卡片可经指针打开现有 Workspace picker，只读 textarea 也可通过 Enter 或 Space 打开。禁用控件会把指针事件交给卡片，卡片也会拦下 `pointerdown`，避免已打开 picker 的外点关闭与重新打开发生竞态。它不会换入一棵平行树，因此选择 Workspace 时 textarea DOM 不会被销毁；严格会话作用域的控件 seat 在会话存在之前保持为空。

Composer 下方的官方 0.1.5 统计改为居中的两枚胶囊，而不再是一整条可能溢出的文字。仪表胶囊显示轮次／步骤与吞吐，点击后展开 LLM 用时、工具用时、平均首 token 延迟和速度；数据库胶囊显示总 token 与缓存命中率，点击后展开未缓存输入、缓存读取、缓存写入和输出的精确计数。两个锚定弹窗互斥，Escape 或点击外部会关闭。两枚胶囊仍通过标准套件读取全日志 `sessionStats` 与 token-meter 的 `tokenUsage` 投影；只有缺少 `sessionStats` 时才回退可见窗口折算。每个已结算轮次仍在 assistant footer 中提供 hover 可见的 TTFT／吞吐标签。上下文占用率继续位于 composer 尾部的 14px ContextMeter，由 `contextPressure` 与 `contextBreakdown` 驱动，与两枚会话统计胶囊相互独立，并继续明确为近似值（[原理](../../llm/token-meter/README.md)）。
...
- **工具详情已退役**：原始选中调用面板、`conversation.details.tool` slot 以及 ui-tool 的惰性注册已随 Workbench 成为根详情区唯一 occupant 而全部移除；审查功能后续作为专用 Provider 重做。
