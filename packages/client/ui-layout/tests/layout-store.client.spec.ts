...
} from '@ryanyujazz/dsh-client-ui-layout/src/client/columns.ts'
...
describe('createLayoutStore', () => {
  ...
  it('has no stage-mode state at all (the app stage was removed)', () => {
    const { store, actions } = createLayoutStore().create()
    expect(store.getSnapshot()).not.toHaveProperty('stageMode')
    expect((actions as Record<string, unknown>).setStageMode).toBeUndefined()
    expect((actions as Record<string, unknown>).setDockOpen).toBeUndefined()
    expect((actions as Record<string, unknown>).setStageActivity).toBeUndefined()
  })
})
