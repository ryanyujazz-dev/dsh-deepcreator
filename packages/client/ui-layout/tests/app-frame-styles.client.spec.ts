...
describe('AppFrame native title-bar drag region', () => {
  ...
  it('carries no app-stage takeover CSS at all', () => {
    expect(css).not.toContain('appsLayer')
    expect(css).not.toContain('activityChip')
    expect(css).not.toContain('dockMask')
    expect(css).not.toContain('data-stage-mode')
    expect(css).not.toContain('data-dock-open')
    expect(css).not.toContain('data-dock-details')
    expect(css).not.toContain('--dsh-dock-width')
  })
})
