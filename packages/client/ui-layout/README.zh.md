...
## 手机全 Stage Workbench 投影与浏览器历史账本

手机端的全 Stage details 投影（打开的 Workbench 覆盖整个 Stage）参与一份浏览器历史账本：进入时压入一条带标记的条目，系统返回手势经 popstate 弹回，监听器只关闭标记离开栈顶的那一层；程序化退出仅在自有条目仍居栈顶时消费它。被掩埋的历史条目保持无害。框架卸载时会回收账本，带标记的条目绝不比它的层活得更久。
