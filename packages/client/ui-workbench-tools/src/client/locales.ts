...
export const zh = {
  ...
  'terminal.sessions': '终端会话', 'terminal.new': '新建', 'terminal.kill': '终止', 'terminal.legacy': '这是旧的逐行终端会话；请新建终端以使用完整交互模式。', 'terminal.noBackend': '没有可用的终端后端。', 'terminal.closePrompt': '是否终止这个终端？选择“取消”只隐藏标签，终端继续运行。',
...
export const en: Record<ToolsKey, string> = {
  ...
  'terminal.sessions':'Terminal sessions','terminal.new':'New','terminal.kill':'Kill','terminal.legacy':'This is a legacy line terminal. Create a new terminal for full interaction.','terminal.noBackend':'No terminal backend is available.','terminal.closePrompt':'Terminate this terminal? Cancel only hides the tab and keeps the terminal running.',
