 ...
 * document. The plugin also registers the Appearance preference row and the
 * content font-size row (official rc.1) into the settings surfaces — the
...
import {
  ...
  DEFAULT_CODE_FONT, DEFAULT_DARK_CODE_THEME, DEFAULT_FONT_SIZE, DEFAULT_LIGHT_CODE_THEME,
...
const TRANSCRIPT_TOKENS: Readonly<Record<TranscriptTextSize, ThemeTokens>> = Object.freeze({
  small: Object.freeze({
    '--dsw-font-markdown-base': '400 13px/20px var(--dsw-font-family)',
    '--dsw-font-markdown-base-strong': '600 13px/20px var(--dsw-font-family)',
    '--dsw-font-markdown-base-italic': 'italic 400 13px/20px var(--dsw-font-family)',
    '--dsw-font-markdown-base-strong-italic': 'italic 600 13px/20px var(--dsw-font-family)',
    '--dsw-font-markdown-base-font-size': '13px',
    '--dsw-font-markdown-base-line-height': '20px',
    '--dsw-font-markdown-code': '400 11px/20px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block': '400 11px/20px var(--ds-font-family-code)',
    ...
    '--dsw-font-sidebar-font-size': '13px',
    '--dsw-font-sidebar-line-height': '20px',
    '--dsw-sidebar-row-height': '28px',
  }),
  standard: Object.freeze({
    ...
    '--dsw-font-markdown-base-italic': 'italic 400 14px/22px var(--dsw-font-family)',
    '--dsw-font-markdown-base-strong-italic': 'italic 600 14px/22px var(--dsw-font-family)',
    '--dsw-font-markdown-base-font-size': '14px',
    '--dsw-font-markdown-base-line-height': '22px',
    '--dsw-font-markdown-code': '400 12px/22px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block': '400 12px/22px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block-small': '400 12px/18px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block-small-line-height': '18px',
    '--dsw-font-markdown-code-block-small-font-size': '12px',
    '--dsw-font-sidebar-font-size': '14px',
    '--dsw-font-sidebar-line-height': '22px',
    '--dsw-sidebar-row-height': '30px',
  }),
  large: Object.freeze({
    '--dsw-font-markdown-base': '400 15px/24px var(--dsw-font-family)',
    '--dsw-font-markdown-base-strong': '600 15px/24px var(--dsw-font-family)',
    '--dsw-font-markdown-base-italic': 'italic 400 15px/24px var(--dsw-font-family)',
    ...
    '--dsw-font-markdown-code': '400 13px/24px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block': '400 13px/24px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block-small': '400 13px/20px var(--ds-font-family-code)',
    '--dsw-font-markdown-code-block-small-line-height': '20px',
    '--dsw-font-markdown-code-block-small-font-size': '13px',
    '--dsw-font-sidebar-font-size': '15px',
    '--dsw-font-sidebar-line-height': '24px',
    '--dsw-sidebar-row-height': '32px',
  }),
})
...
export class ThemeRuntime {
  ...
  exportInspectTokens(): ThemeTokenInspection[] {
    ...
    for (const name of Object.keys(TRANSCRIPT_TOKENS.standard)) {
   ...
   * entry (official rc.1). Accepted values are written through the settings
  ...
  /**
   * Switch the transcript typography preference and persist it through the
   * settings scope. The published theme snapshot carries concrete font tokens
   * so consumers render the new size without page or subtree scaling.
   * @param size - selected small, standard, or large transcript role.
   */
  setTranscriptTextSize(size: TranscriptTextSize): void {
    if (this.transcriptTextSize === size) return
    this.transcriptTextSize = size
    void this.host.set(TRANSCRIPT_TEXT_SIZE_FIELD, size)
    this.publish()
  }
   ...
   * then apply the user's transcript role as the typography authority.
   */
 ...
 * ran (official rc.1), so the initial snapshot matches first paint and
 * ui-layout's presenter does not flash the schema default while the settings
 * read is in flight. Non-browser runs and mounts without the boot script fall
 * back to the schema default; the durable settings adoption still lands
 * afterwards.
 */
...
export function apply(ctx: ClientContext): void {
  const host = ctx.settingsScope.bind<ThemeSettings>({
    ...
    decode: (value) => {
      ...
      // Documents persisted before the font-size axis carry no field; an
      // out-of-range one is treated the same as absent (schema default).
      const fontSize = candidate.fontSize
      return {
        ...candidate,
        fontSize: typeof fontSize === 'number' && Number.isInteger(fontSize)
