...
describe('ThemeRuntime', () => {
  it('defaults to the system preference resolved against prefers-color-scheme', () => {
    ...
    expect(snapshot.transcriptTextSize).toBe('standard')
  ...
  it('sets and adopts transcript text size through real typography tokens', () => {
    const { theme, events, host } = make()
    theme.setTranscriptTextSize('large')
    expect(theme.getTheme().transcriptTextSize).toBe('large')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base']).toBe('400 15px/24px var(--dsw-font-family)')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-code']).toBe('400 13px/24px var(--ds-font-family-code)')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base-font-size']).toBe('15px')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base-line-height']).toBe('24px')
    ...
    expect(theme.getTheme().transcriptTextSize).toBe('small')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base']).toBe('400 13px/20px var(--dsw-font-family)')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base-font-size']).toBe('13px')
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base-line-height']).toBe('20px')
  ...
  it('keeps transcript prose synchronized with the matching sidebar role', () => {
    const { theme } = make()
    for (const [size, fontSize, lineHeight, codeSize] of [
      ['small', '13px', '20px', '11px'],
      ['standard', '14px', '22px', '12px'],
      ['large', '15px', '24px', '13px'],
    ] as const) {
      theme.setTranscriptTextSize(size)
      const tokens = theme.getTheme().active.tokens
      expect(tokens['--dsw-font-markdown-base-font-size']).toBe(fontSize)
  ...
  it('replacing one source leaves its stale disposer harmless', () => {
    ...
    current()
    current()
    expect(theme.getTheme().active.tokens['--new']).toBeUndefined()
    expect(theme.getTheme().active.tokens['--dsw-font-markdown-base-font-size']).toBe('14px')
    expect(theme.getTheme().active.tokens['--dsw-font-sidebar-font-size']).toBe('14px')
    expect(theme.getTheme().active.tokens['--dsw-sidebar-row-height']).toBe('30px')
