...
同一个功能自有「偏好」分组除了把 `ui-theme.transcriptTextSize` 持久化为 `small`、`standard` 或 `large`，还分别保存浅色／深色代码主题和一个代码字体预设。代码外观提供 DeepCreator、Shiki 的 GitHub Default、One Light/Dark，以及六套第三方开源主题（Catppuccin Latte/Mocha、Rosé Pine Dawn/Moon、Vitesse、Kanagawa Lotus/Wave、Everforest Light/Dark 与 Tokyo Light/Night）；设置中的固定 TypeScript Diff 预览始终强制使用其正在配置的明暗主题，预览背景、行号和增删符号均在该局部主题内隔离，不受应用当前明暗模式影响。代码字体可选 System Mono、JetBrains Mono、Fira Code 与 Source Code Pro；三款开源字体以本地资源打包，选中的字体栈同时发布到 `--ds-font-family-code` 和 `--dsw-font-mono`，供 Markdown 代码、Read、Diff、行内代码和 xterm 使用。ThemeRuntime 会在不可变快照中发布当前代码主题，ui-layout 再把它投射到 `body[data-code-theme]`。每个代码主题拥有自己的编辑器前景／背景和半透明的行级／词级 Diff 表面；第三方主题的表面逐项直接绑定上游 `diffEditor.*` 值，仅当上游没有定义该可选角色时保留产品回退。行号、增删符号、控件与其他 chrome 仍使用应用 token。Token selector 必须在各自 `[data-code-theme='...']` 分支里直接消费 Shiki span 自身的变量，不能再经由父级作用域里的“active token”自定义属性中转。

ThemeRuntime 会在配色与扩展 token 组合完成后，把文字尺寸偏好投射为具体的 Markdown 正文、对话代码以及侧边栏排版与行高 token，因此 ui-layout 调整的都是真实尺寸，不会缩放页面。展开 Tool 载荷使用的紧凑 code-block-small 角色保持为 12px／18px。该分组把当前平台的「系统字体」栈显示为界面字体值；代码字体单独配置。它声明并渲染 `deepcreator.settings.preferences.item`，功能插件可以追加设置行，同时继续拥有自己的标签、持久化字段和实时状态。对话正文、侧边栏与对话标题栏文字共用 13px／20px、14px／22px 或 15px／24px，侧边栏行分别为 28px、30px 或 32px。普通界面、对话正文和代码角色使用 400 字重，明确强调仍使用 500 或 600。设置页和 Menu 的界面角色保持为 12px／18px。这些可复用角色由 [Web UI 样式参考](../../../docs/web-styling.md)统一规定。
...
`src/styles/` 下有五张全局样式表，由主题客户端入口自身按顺序导入：`base.css`、`design-platform.css`、`scrollbar.css`、`gradient-shadow-text.css` 与 `shiki.css`。主题包直接持有这个入口，不再依赖呈现包传递进来的 CSS 副作用。`scrollbar.css` 是 `--dsw-alias-scrollbar-*` token 的唯一消费方，必须排在声明这些 token 的 `design-platform.css` 之后。

`base.css` 同时拥有共享的 `--dsh-reading-content-width` 阅读宽度（748px）。Conversation 从该 token 派生对话正文宽度，Artifact Markdown 等文档型预览将其作为居中的最大宽度，同时保持面板滚动表面为全宽。
