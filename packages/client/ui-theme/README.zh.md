...
同一个功能自有「偏好」分组将 `ui-theme.transcriptTextSize` 持久化为 `small`、`standard` 或 `large`。ThemeRuntime 会在配色与扩展 token 组合完成后，把该偏好投射为具体的 Markdown 正文、对话代码以及侧边栏排版与行高 token，因此 ui-layout 调整的都是真实尺寸，不会缩放页面。展开 Tool 载荷使用的紧凑 code-block-small 角色保持为 12px／18px。该分组只把当前平台的「系统字体」栈显示为界面字体值，不会呈现不可用的字体选项。它声明并渲染 `deepcreator.settings.preferences.item`，功能插件可以追加设置行，同时继续拥有自己的标签、持久化字段和实时状态。对话正文、侧边栏与对话标题栏文字共用 13px／20px、14px／22px 或 15px／24px，侧边栏行分别为 28px、30px 或 32px。普通界面、对话正文和代码角色使用 400 字重，明确强调仍使用 500 或 600。设置页和 Menu 的界面角色保持为 12px／18px。这些可复用角色由 [Web UI 样式参考](../../../docs/web-styling.md)统一规定。
