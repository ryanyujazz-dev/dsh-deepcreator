...
import { TEST_SCHEMA, TestDescribeMirror } from './describe-mirror.fixture.ts'
...
const schema: SettingsSchemaService = TEST_SCHEMA as unknown as SettingsSchemaService
...
function permissionController(api: object) {
  ...
  const mirror = new TestDescribeMirror(ctx)
...
describe('permission settings store', () => {
  ...
  it('hides the row in a remote browser instead of loading forever', async () => {
    ...
    const mirror = new TestDescribeMirror(ctx, 'memory')
