...
`workbench-artifact` 节点完全遵循官方 `ui-deliverables` 的推导规则:`turn/start` 启动每轮上下文,`tool/call` 记录调用视图,append 表面的 `tool/result` 收集 diff 卡片(或 generic `edit` 卡片)产出的路径——读取、删除、失败结果与替换表面不产生产物。`artifacts` 快照 builder 把每轮节点折叠为每条路径一条记录(最新产出胜出),按产出时间倒序排列。DeepCreator 禁用官方重复的产物尾行，由新的 Turn 变更卡接管可见尾部；本投影继续发布 Turn location data 与 closing prose 的 `chatFileMentions`，保证正文中的唯一文件名或完整路径仍可点击。
