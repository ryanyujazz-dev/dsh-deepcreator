...
读取边界返回带类型的展示载荷，不再把所有文件都按 UTF-8 解码。图片通过工作区围栏保护的 loopback URL 直接在 Artifact 实例中显示；PDF 也留在同一个实例内，由 Chromium 内嵌 PDF 渲染器通过 byte-range 响应消费该 URL，iframe 独占内容 viewport 与滚动，Artifact 外层不再叠加第二套滚动。DOCX 由 Mammoth 转为结构化 HTML 后放进禁用脚本的 sandbox iframe；旧 DOC 由 `word-extractor` 提取正文，并在文档阅读表面中展示。这些路径都不会激活 Browser 面板。HTML／HTM 是下文所述的明确例外：只有轮尾产物卡行内“打开”操作会在 Browser 中运行页面，点击文件主体或产物面板首页行仍在 Artifact 查看源码。
