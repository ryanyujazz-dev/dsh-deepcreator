...
读取边界返回带类型的展示载荷，不再把所有文件都按 UTF-8 解码。图片通过工作区围栏保护的 loopback URL 直接在 Artifact 实例中显示；PDF 也留在同一个实例内，由 Chromium 内嵌 PDF 渲染器消费该 URL。DOCX 由 Mammoth 转为结构化 HTML 后放进禁用脚本的 sandbox iframe；旧 DOC 由 `word-extractor` 提取正文，并在文档阅读表面中展示。这些路径都不会激活 Browser 面板。HTML／HTM 是下文所述的明确例外：只有行内“打开”操作会在 Browser 中运行页面，点击文件主体仍在 Artifact 查看源码。
...
HTML／HTM 仍然是同一份官方产物列表中的普通文件，不引入第二套产物注册表或事件。它们的首页文件行最右侧增加分裂式「打开」控件：主按钮及菜单中的「在 DeepCreator 中打开」先从 `remote.artifacts.preview` 获取经过工作区围栏的 loopback 预览 URL，再通过公共 Presentation Client 显式指定 `browserId: "iab"`；Browser URL resolver 创建准确的内置浏览器 Tab，现有 Workbench Browser Presenter 负责可见性与挂载回执。「在系统浏览器中打开」则把真实 HTML 路径交给官方 Workspace／OS 路径开启器。点击分裂控件之外的文件行仍打开只读源码 Artifact 标签。
