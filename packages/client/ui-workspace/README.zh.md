...
浏览器根节点与独立的会话置顶区都把外壳的 `--dsh-sidebar-section-margin-top` token（10px，并保留相同的本地回退值）作为顶部外边距，不设置底部外边距，分区标题也不再叠加顶部外边距。置顶区是 Workspace 浏览器上方的同级分区，最多占侧边栏可用高度的 40%、拥有独立滚动，并在搜索期间隐藏。
