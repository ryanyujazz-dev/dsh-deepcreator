...
import { PreferencesGroup } from '../src/client/PreferencesGroup.tsx'
...
describe('ui-settings-general apply', () => {
  ...
  it('fills all five seats for declarations before or after apply', async () => {
    ...
    expect(before.slots.entries('settings.general.item')[0]!.component).toBe(PreferencesGroup)
    expect(before.slots.spec('deepcreator.settings.preferences.item')).toEqual({ kind: 'list', scope: 'root' })
  ...
  it('re-registers after an HMR collapse of the declaring chain (stale disposers must not block)', async () => {
    ...
    expect(b.slots.entries('settings.general.item')[0]!.component).toBe(PreferencesGroup)
