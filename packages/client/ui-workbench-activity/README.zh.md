...
- **Home** —— 锚定对话宿主会话的单页纵排视图(子代理经「在对话区打开」
  ...
  **本轮参与**(最近活跃晚于父会话最近一条
  ...
  路径 `sessions.open(parent)`);点击卡片仍进入其标签页。嵌套子代理默认
  展开:官方目录条目带 `hasChildren` 的卡片直接以左侧垂线引导缩进递归显示
  子级,无需点击;箭头用于收起。开放层级由官方目录减去仅呈现用的「已收起」
  集合派生,某分支首次获得子代理时会自行展开。每个开放层级调用官方
  `sessions.setSubagentCatalogOpen(child, true)` —— 加载该层的
  `subagentsByParent` 目录并在开放期间保持实时 —— 收起或隐藏面板即释放
  订阅(收起祖先连同更深层级一并释放)。面板自身不保存层级:行、标签、
  模式、活动位与「可展开」提示全部来自官方逐层目录。嵌套卡片以自己的
  会话 id 打开 Workbench 标签页(收起后标签名仍在,因为官方目录保持已
  加载),其实例工具条按目录中解析到的精确直接父地址跳转对话区。
