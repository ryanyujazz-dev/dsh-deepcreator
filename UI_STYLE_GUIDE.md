...
- 五类产品图标保持极简语义：Activity 为两行且每行都是「小圆圈＋横线」，Artifact 为带夹板的剪贴板（顶部夹子＋两条内容横线），Review 为圆角方框内的上下 2:1 分区——加号占上方约 2/3、减号占底部约 1/3，Terminal 为无外框的折角提示符与一条横线，Preview 使用产品提供的 `play_fill.svg` 原始轮廓，转换为空心 `currentColor` 描边后在 16px 画布内居中缩放至 82%。合并态「面板」按钮继续使用项目内统一的 Workbench 面板图标。
