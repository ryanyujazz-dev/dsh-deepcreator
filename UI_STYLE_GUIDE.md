...
- Review 标题右侧使用同一 Header 内的紧凑范围下拉，不另起工具栏；顺序固定为「未暂存」「已暂存」「未提交」和按新到旧排列的「历史轮次 · TURN N」，缺省「未提交」，历史菜单只列仍有待处理文件的轮次。文件 Header 不显示 `M` 等状态字母。最终回答后的变更卡使用 1px `border-l1`、9px 圆角和 elevated token 表面，主区为图标＋12px/500 文件数＋11px remaining/status，右侧用分隔线切出 11px「撤销」「审查」动作；已解决卡整体降透明度且主区、审查均禁用。只有最新未解决 Turn 的撤销可用，确认与错误分别复用 Modal 和 Toast。不得为卡片新增硬编码颜色。
