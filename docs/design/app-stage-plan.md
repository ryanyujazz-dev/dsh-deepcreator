...
Status: **执行中 — M5 Px-β+Px 通道层完成（M5a-d 全部提交并实机验收：壳层投影含用户目击、注入 runtime+SSE 含涟漪复验；共揪出三真缺陷均已修复），余合成光标（hover/拖拽态，低优先）与 Px-γ Desktop 投射（随 IAB 分支）；时间线活动视图已完成（M5e，f8ab1fb+fix）；键入参数摘要·非共见保守型已完成（M5f，c9d018f+fix）；共见键入 ghost·X7 完整型已完成（M5g）**（M4：桥 v2 路由 + 九工具 + 资产通道 + 通道一注册闸 + 活动信号；GUI 真实验收全通——preset 会话四步：createTask 落盘（created + persistedKeys + 版本回执）/ data_read 验证 / dev: 拒收 / app_open；活动 chip 由用户在自己的 GUI 面上亲眼确认（多面认领归最新 poller 的实测副产品：用户面常胜，chip 亮起即三层验证）；验收揪出三个真缺陷并修复：桥双绑 f444a28、宿主侧路由扇出双投递 22b8589（routerId 认领制，hub 四测含 poll-gap 边界）、demo 协议卫生 v0.2.1（已发布安装）；245 文件 / 2611 测试全绿；新宿主冒烟 rev11→12 恰好一写、多轮 8 连发全单写）。本文把 `docs/design/app-stage.md` v0.0.6 的阶段划分展开为可执行的开发计划：里程碑、包结构与行序、每步的产出与验收、验证纪律。设计依据四份文档：主提案（决策）、`app-stage-agent-surface.md` v1.0（工具/技能）、`app-stage-ui.md` v1.0（前端）、`app-stage-presence.md` v0.0.3（在场层）。计划执行中以实现事实为准回填，与设计冲突时先回设计文档裁决再改代码。
...
**M5d 实现记录（注入通道，两提交）**：coordinator 增事件总线（lease/command 结构化事件，seq 递增、环形 100 条、per-app 过滤订阅）；静态源三新面——HTML 注入（`</head>` 前插同源 defer runtime，data-dsh-app 绑定自 installed id/dev 目录名，属性转义+无头文档回退）、冻结 runtime 路由、SSE `/__dsh_presence__/events`（重放窗+seq 去重、先订阅后重放封竞态、25s 心跳、close 退订、CORS * ——沙箱帧是 opaque origin 且流是只读结构化事实）；注入 runtime 不变量（§7）：零可调用 API、单向数据流、closed shadow root + pointer-events:none、通道态显式 data-dsh-presence 属性（connecting/live/degraded/unsupported）、reduced-motion 显式降级、无 app 绑定不画（缺元数据绝不编造视觉）。
**M5d 实机验收记录（2026-08-26 深夜，三次宿主重启）**：①curl——注入标签带绑定出现在已安装应用 HTML、runtime 脚本可取、SSE 流式响应；②preset 会话 takeover+createTask——SSE 流全事件到达（lease active → command start → settled，字段精确 macro/delegated/invoke/createTask/ok）；③应用文档直开——presence=live、注入标签+绑定在档；④**涟漪复验**——首个 runtime 把 phase 读错层级（event.phase vs event.payload.phase，事件全到但过滤恒 false 涟漪永不画；页面内手工 replica 收到全部事件而 runtime host 永不出现定位了缺陷在终端过滤），修复后 arm MutationObserver 触发 createTask：**rippleFired=1 涟漪真实渲染**。测试基线 248 文件/2654 绿。

**M5e 实现与验收记录（2026-08-26 深夜，两提交）**：活动时间线=Launcher 右上「活动」按钮（与开发中菜单同簇）+8px 业务蓝点+浮层面板（行=时间 tabular-nums/应用名/action 名或类型/结果状态右置，完成三级、失败错误色）；全局单一水位 `activity-seen.json`（latest-seen 即未读数，宿主重启存活），feed 只聚合 installed origin。实机验收发现并修复两真缺陷：①蓝点轮询误挂 scanTick（只在发布重扫时 +1）——改挂 presence feed 的 activity 订阅（命令 settle 即 bump）；②打开面板推进水位但本地未读不清零（面板开着蓝点仍亮）——open 即 read，markSeen 落地即熄灭。终验：新卡→蓝点 1 ✓、面板 3 行 ✓、开面板蓝点熄 ✓、水位持久化 seq ✓。测试基线 248 文件/2657 绿。

**M5f 实现与验收记录（2026-08-27 凌晨，两提交）**：键入 ghost 按 presence 文档 §3.2 共见分型的保守侧落地——非共见型（共见检测=runtime 上报 AppData 订阅事实，后续轮次）：横幅内联结构化参数摘要（参数名三级+值次级 tabular-nums、单值 24ch 省略、整块 46ch、宿主截断每值 120 字/4 对，JSON 渲染复合值），**不做光 cursor 移动与键入剧场**；摘要只走壳层权威快照（lease.activeCommand），**永不进应用层 SSE 通道**（自由文本纪律，测试钉死）；数据源只来自下行 invoke 参数。实机验收揪出两真缺陷：①摘要原设计 in-flight-only，实测 invoke 仅 ~100ms（SSE 时间戳 094→201），对 2s 快照轮询几乎必然错过——摘要事实上永不可见；修复=settled 后保留至下条命令替换（语义=「最近命令的参数」事实而非「正在执行」声明，waiting-user 仍隐藏）；②host remote 结果类型加字段后 workbench-remotes 未重建——typert 生成 schema 在 wire 上剥掉未知字段 activeCommand（重建链再次咬人，已记入教训清单）。终验：横幅内 column=todo、title=M5f复验摘要 双参数对真实渲染。

**M5g 实现与验收记录（2026-08-27 凌晨）**：共见键入 ghost（X7 完整型）——宿主判共见（应用桥 dataChanges 轮询即订阅事实，15s 新鲜窗；零新增线面，上行 POST 保留给只有 runtime 能观察的事实）时，app-scoped SSE command.start 携 params（§4.2 下行白名单：值的终点就是该视口）；注入 runtime 演出：合成光标（视口左下 320ms ease 缓动至中心，无伪造路径）+ ghost（action 名大写蓝标签 + 值胶囊 30–60ms/字限速逐字，权威 settle 快进）+ commit 闪动/红停/3s 点线等待态；reduced-motion 不演；宿主不判共见则 runtime 永不渲染——剧场无法被应用侧触发。实机验收：开容器（订阅活）→ 建卡 → SSE 实录 `params:[{column:todo},{title:M5g共见ghost}]` 下行 ✓、卡落盘 ✓、直开应用页 ghost host div 真实挂载 ✓。presence 文档升 v0.0.4（宿主侧检测源/settle 快进/摘要持久化三条实现修订）。测试基线 248 文件/2664 绿。
