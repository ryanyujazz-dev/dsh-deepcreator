...
For live IAB content the UI additionally supplies a `BrowserSurfaceBridge`; snapshot Providers need only render URL, state, action history, and the latest event-driven screenshot. Screenshot bytes are saved exactly once in the official Attachment Store. `BrowserTabState.snapshotAttachment`, the screenshot Tool Content image block, Session replay, and Browser Panel all reference the same `attachmentId`; `snapshotArtifactId` is a one-release compatibility alias only. The atomic Browser state snapshot never carries multi-megabyte image data. Preview hydration reads the attachment through an agent-fenced Remote, publishes independently of Host revision, reports fetch errors instead of presenting them as “not yet captured”, and uses bounded automatic plus explicit retries. New screenshots never create `$DSH_HOME/artifacts/browser-screenshots` files.
